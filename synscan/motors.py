...
class Motors(UdpCommunicationsModule):
    """
    ...
    * For GOTO mode, the motor controller will take care of T1 automatically,
        but motion mode has to be set to tracking = False
    ...
    Typical session:
        ...
        * For a GOTO slewing, check the motor status to confirm that the motor stops (Generally means arriving
            the destination.)
    ...
    **NOTE:** Methods begining with axis prefix act only onto selected axis.
    ...
    def _init(self):
        ...
        try:
            ...
            logging.warning(f'Retriying in {retry_sec}..')
            time.sleep(retry_sec)
            self._init()

    def _degrees_per_second_to_t1_preset(self, axis, degrees_per_second):
        counts_per_second = self.degrees2counts(axis, degrees_per_second)
        tmr_freq = self.params[axis]['TimerInterruptFreq']
        if abs(counts_per_second) <= 0:
            t1_preset = tmr_freq
        else:
            t1_preset = tmr_freq / abs(counts_per_second)
        return t1_preset

    def get_values(self, parameter_dict):
        """
        ...
        for axis in range(1, 3):
            params[axis] = dict()
            for parameter, cmd in parameter_dict.items():
                try:
                    params[axis][parameter] = self.send_command(cmd, axis)
                except NameError as error:
                    logging.warning(error)
    ...
    def get_parameters(self):
        ...
        try:
            params = self.get_values(parameter_dict)
        except NameError as error:
            logging.warning(error)
    ...
    def axis_set_motion_mode(self, axis, Tracking, CW=True, fastSpeed=False):
        ...
        response = self.send_command('G', axis, value, ndigits=2)
        return response
    ...
    def axis_set_goto_targetCounts(self, axis, targetCounts):
        ...
        # Position values are offseting by 0x800000
        response = self.send_command('S', axis, targetCounts + 0x800000)
        return response
    ...
    def axis_set_posCounts(self, axis, counts):
        ...
        # Position values are offseting by 0x800000
        response = self.send_command('E', axis, counts + 0x800000)
        return response
    ...
    def axis_set_speed(self, axis, degreesPerSecond):
        ...
        if degreesPerSecond != 0:
            response = self._set_T1_preset(axis,
                                           int(self._degrees_per_second_to_t1_preset(axis, abs(degreesPerSecond))))
        else:
    ...
    def axis_start_motion(self, axis):
        """Start Goto"""
        response = self.send_command('J', axis)
    ...
    def axis_stop_motion(self, axis, syncronous=True):
        ...
        response = self.send_command('K', axis)
        if syncronous:
    ...
    def set_switch(self, on):
        ...
        response = self.send_command('O', 1, value, ndigits=1)
        return response
